...
def odometry_callback(data,args):
    ...
    diff_x = float(target_x) - float(y.pose.pose.position.x)
    diff_y = float(target_y) - float(y.pose.pose.position.y)
    # Controladores PID para posicion X e Y
    u_x = pid_x.update(y.pose.pose.position.x, float( str(y.header.stamp.secs) +"." + str(y.header.stamp.nsecs) ) )
    u_y = pid_y.update(y.pose.pose.position.y, float( str(y.header.stamp.secs) +"." + str(y.header.stamp.nsecs) ) )
    rospy.loginfo("***********************")
    rospy.loginfo(u_x)
    rospy.loginfo("***********************")
    rospy.loginfo(u_y)
    ...
    quaternion = (y.pose.pose.orientation.x,y.pose.pose.orientation.y,y.pose.pose.orientation.z,y.pose.pose.orientation.w)
    ...
    #theta_roll_d  = 0.09*(Ax*math.sin(euler[2]) - Ay*math.cos(euler[2])) - u_y + euler[0]
    #omega_pitch_d = 0.09*(Ax*math.sin(euler[2]) + Ay*math.cos(euler[2])) - u_x + euler[1]
    if (diff_y > diff_y*0.75):
        theta_roll_d  =  euler[0] - u_y 
    else:
        theta_roll_d  =  euler[0] + u_y 
    #theta_roll_d  =  euler[0] - u_y 
    if theta_roll_d > 0.3490:
        theta_roll_d = 0.3490
    elif theta_roll_d < -0.3490:
        theta_roll_d = -0.3490 

    #omega_pitch_d =  euler[1] - u_x 
    omega_pitch_d = 0
    if omega_pitch_d > 0.3490:
        omega_pitch_d = 0.3490
    elif omega_pitch_d < -0.3490:
        omega_pitch_d = -0.3490
...
def talker():
    ...
    pub = rospy.Publisher('/cygnus/command/roll_pitch_yawrate', RollPitchYawrateThrust, queue_size=20)
